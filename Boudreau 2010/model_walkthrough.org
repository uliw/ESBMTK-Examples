...
M = Model(
    stop="100 ky",  # end time of model
    max_timestep="1 ky",  # time step
...
Running the model and displaying results is also straightforward 
#+BEGIN_SRC ipython
M.run()
M.plot([M.CO2_At, M.L_b, B.D_b])
#+END_SRC
...
#+END_SRC
once this is saved to e.g., =boudreau_2010.py=, we can import the model definition like this into a task-specific python script. We use this approach to create i.e., the steady-state conditions or to drive the model with a carbon pulse.
#+BEGIN_SRC ipython
